...
import { WardrobeLabScene } from "@/components/avatar-v5-wardrobe-lab/wardrobe-lab-scene";
import type {
...
const views: Array<{ label: string; view: AvatarView }> = [
  { label: "Front", view: "front" },
  { label: "Side", view: "side" },
  { label: "Rear", view: "back" },
];

export default function WardrobeLabStudio({
  selection,
  approvedHair,
  beard,
  appearance,
  ...
  return (
    <div
      ref={containerRef}
      className="relative isolate h-[34rem] min-h-[28rem] w-full min-w-0 overflow-hidden rounded-[32px] border border-white/10 bg-[#0b1018] lg:h-[43rem]"
    >
      <Canvas
        dpr={dpr}
        shadows
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between gap-3 p-4 sm:p-5">
        <div className="rounded-full border border-white/10 bg-black/30 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-white/55 backdrop-blur">
          {quality} quality · isolated audit
        </div>
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-wrap items-center justify-center gap-1.5 bg-gradient-to-t from-black/75 via-black/30 to-transparent px-4 pb-4 pt-12">
        ...
        <button
          ...
          className="rounded-full border border-white/12 bg-black/38 px-3 py-2 text-xs font-semibold text-white/72 backdrop-blur transition hover:border-white/25 hover:text-white disabled:opacity-35"
        >
